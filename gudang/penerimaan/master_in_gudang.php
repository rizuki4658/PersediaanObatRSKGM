...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Master Penerimaan Gudang</li>
</ol>
 <br>
...
<?php
    ...
    $pencarian="SELECT * FROM in_gudang WHERE kode_p like '%$cari%' or kode_obat like '%$cari%' or nama_obat like '%$cari%' or jumlah like '%$cari%' or satuan like '%$cari%' or nama_pet like '%$cari%' or ruangan like '%$cari%'";
    $result = $link->query($pencarian);
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link, $pencarian ."LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_gudang ORDER BY tanggal DESC";
    ...
    $page = isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
    $mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;
    $result = mysqli_query($link,"SELECT * FROM in_gudang");
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link,"select * from in_gudang LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_gudang ORDER BY tanggal DESC";
	$execute2=mysqli_query($link,$query2);	
	} 
...
<div class="container" style="margin-left: 20px;">
	...
	<div class="card mb-3">
		...
		<div class="card-header">
			<i class="fa fa-table"></i>&nbsp;Tabel Penerimaan Gudang
		</div>
			<!--Div Ke-empat-->
			<div class="card-body">			
				<!--Div Ke-Lima-->
					...
					<table width="100%" cellspacing="0">
						<tr>
		              		<th colspan="5" style="width: 700px;">
		              			<a class="btn btn-primary btn-md" href="tambah_in_gudang.php" <?php if ($_SESSION['ses_level']!='Gudang') {
					 ...
					 ?>
						<tbody>
							<tr>
								...
								<td><center><?php echo $data['jumlah1']; echo " ".$data['satuan1']; ?></center></td>
								<td><center><?php echo $data['jumlah2']; echo " ".$data['satuan2']; ?></center></td>
								<td><?php echo $data['nama_pet']; ?></td>
								<td><center><?php echo $data['ruangan']; ?></center></td>
								<td>
				                  	<center>
				                  	<a title="Edit" href="<?php echo "edit_in_gudang.php?kode_p=$data[kode_p]";?>" class="btn btn-success btn-sm"  <?php if ($_SESSION['ses_level']!='Gudang') {
		              				echo "style='pointer-events:none; opacity:0.5;'";
		              			}else{
		              				echo "";
		              			}?>>
				                  	...
				                  	<a title="Hapus" onclick="return confirm('Anda Yakin Akan Menghapus data?')" href="<?php echo "delete_in_gudang.php?kode_p=$data[kode_p]";?>" class="btn btn-danger btn-sm"  <?php if ($_SESSION['ses_level']!='Gudang') {
		              				echo "style='pointer-events:none; opacity:0.5;'";
		              			}else{
		              				echo "";
		              			}?>>
