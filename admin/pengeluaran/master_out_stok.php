...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Master Pengeleruan Stok</li>
</ol>
 <br>
...
<?php
    ...
    $pencarian="SELECT * FROM in_stok WHERE (kode like '%$cari%' or kode_pg like '%$cari' or kode_obat like '%$cari%' or nama_obat like '%$cari%' or jumlah_in like '%$cari%' or satuan like '%$cari%' or ruangan like '%$cari%') and ket='Keluar'";
    $result = $link->query($pencarian);
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link, $pencarian ."LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_stok where ket='Keluar' ORDER BY tanggal DESC";
    ...
    $page = isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
    $mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;
    $result = mysqli_query($link,"SELECT * FROM in_stok where ket='Keluar'");
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link,"select * from in_stok where ket='Keluar' LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_stok where ket='Keluar' ORDER BY tanggal DESC";
	$execute2=mysqli_query($link,$query2);	
	} 
...
<div class="container" style="margin-left: 20px;">
	...
	<div class="card mb-3">
		...
		<div class="card-header">
			<i class="fa fa-table"></i>&nbsp;Tabel Pengeluaran Stok
		</div>
			<!--Div Ke-empat-->
			<div class="card-body">			
				<!--Div Ke-Lima-->
					...
					<table width="100%" cellspacing="0">
						<tr>
		              		<th colspan="5" style="width: 700px;">
		              			<a class="btn btn-primary btn-md" href="tambah_out_stok.php" <?php if ($_SESSION['ses_level']!='Admin') {
                          ...
                          echo "";
		              		...
		              		<th>
		              			<form action="master_out_stok.php" method="post">
					 ...
					 ?>
						<tbody>
							<tr>
								...
								<td><center><?php echo $data['jumlah']." ".$data['satuan']; ?></center></td>
								<td><center><?php echo $data['jumlah_out']." ".$data['satuan_out']; ?></center></td>
								<td><?php echo "Rp."." ".number_format($data['harga']); ?></td>
								<td><center><?php echo $data['ruangan']; ?></center></td>
								<td>
				                  	<center>
				                  	<a title="Edit" href="<?php echo "edit_out_stok.php?kode=$data[kode]";?>" class="btn btn-success btn-sm" <?php if ($_SESSION['ses_level']!='Admin') {
                          echo "style='pointer-events:none; opacity:0.5;'";
                        }else{
                          echo "";
                        }?> >
				                  	...
				                  	<a title="Hapus" onclick="return confirm('Anda Yakin Akan Menghapus data?')" href="<?php echo "delete_out_stok.php?kode=$data[kode]";?>" class="btn btn-danger btn-sm" <?php if ($_SESSION['ses_level']!='Admin') {
                          echo "style='pointer-events:none; opacity:0.5;'";
                        }else{
                          echo "";
                        }?> >
