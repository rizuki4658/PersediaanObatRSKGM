...
<ol class="breadcrumb">
	...
	<li class="breadcrumb-item active">Master Pengecekan Penerimaan Pejabat</li>
</ol>
 <br>
...
<div class="container" style="margin-left: 20px;">
    ...
    $pencarian="SELECT * FROM in_pejabat WHERE nama like '%$cari%' or nama_distributor like '%$cari%' or kode_obat like '%$cari%' or hasil_cek like '%$cari%'";
    $result = $link->query($pencarian);
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link, $pencarian ."LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_pejabat ORDER BY tanggal DESC";
    ...
    $page = isset($_GET["halaman"]) ? (int)$_GET["halaman"] : 1;
    $mulai = ($page>1) ? ($page * $halaman) - $halaman : 0;
    $result = mysqli_query($link,"SELECT * FROM in_pejabat");
    $total = mysqli_num_rows($result);
    $pages = ceil($total/$halaman);            
    $query = mysqli_query($link,"select * from in_pejabat LIMIT $mulai, $halaman")or die(mysqli_error);
    $no =$mulai+1;
	$query2="SELECT * FROM in_pejabat ORDER BY tanggal DESC";
	$execute2=mysqli_query($link,$query2);	
	}
	...
	<div class="card mb-3">
        <div class="card-header">
          <i class="fa fa-table"></i>&nbsp;Table Hasil Pengecekan</div>
          	...
          	<table width="100%" cellspacing="0">
          		<tr>
              		<th colspan="5" style="width: 700px;">
              			<a title="Tambah" class="btn btn-primary btn-md" href="tambah_in_pejabat.php"  <?php if ($_SESSION['ses_level']!='Pejabat') {
                          echo "style='pointer-events:none; opacity:0.5;'";
                        }else{
                          echo "";
                        }?> >
              				<i class="fa fa-plus"></i>&nbsp;Pengecekan
              			</a>
              		</th>
              		<th>
            ...
            <table class="table table-bordered" width="100%" cellspacing="0">
              ...
              <tbody>
                <tr>
                  ...
                  <td>
                  	<center>
                  	<a title="Edit" href="<?php echo "edit_in_pejabat.php?id=$data[id]";?>" class="btn btn-success btn-sm" <?php if ($_SESSION['ses_level']!='Pejabat') {
                          echo "style='pointer-events:none; opacity:0.5;'";
                        }else{
                          echo "";
                        }?>>
                  	...
                  	<a title="Hapus" onclick="return confirm('Anda Yakin Akan Menghapus data?')" href="<?php echo "delete_in_pejabat.php?id=$data[id]";?>" class="btn btn-danger btn-sm" <?php if ($_SESSION['ses_level']!='Pejabat') {
                          echo "style='pointer-events:none; opacity:0.5;'";
                        }else{
                          echo "";
                        }?>>
